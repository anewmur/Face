...
class Detector(object):
    ...
    def __init__(self, file_name, target, is_file=True):
        """
        Initialization
    ...
    def run(self, pathOfDumps, writeVideo: object = True) -> object:
        ...
        print('Finished {}.'.format(basename))
        cap.release()

        if not os.path.exists(os.path.join(os.getcwd(), pathOfDumps)):
            os.mkdir(pathOfDumps)


        with open(pathOfDumps + '/' + basename[:-4] + '.json', 'w') as f:
            if len(self.points) > 0:
                json.dump(self.points, f)
                print(' len(self.points) = ', len(self.points))
            
if __name__ == "__main__":
    # file_name = '../../VIDEOS/_FEDOR_/VIDEO/0e1ff690-a98c-11ec-99bc-58a023d3f6d9.mp4'
    file_name = '../../../VIDEOS/_FEDOR_/VIDEO/bc292f06-a9a2-11ec-b5ec-b469216ca443.mp4'
    pathOfDumps = '../../faceDUMPS'
    recog = Detector(file_name,1)
    recog.run(pathOfDumps, writeVideo=False)
